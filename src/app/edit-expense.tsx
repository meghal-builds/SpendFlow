...
export default function EditExpense() {
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState(expenseToEdit ? expenseToEdit.categoryId : 'cat_other');
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState(expenseToEdit ? (expenseToEdit.paymentMethod || 'UPI') : 'UPI');
  const [note, setNote] = useState(expenseToEdit ? (expenseToEdit.note || '') : '');
  const [expenseDate, setExpenseDate] = useState(expenseToEdit ? new Date(expenseToEdit.expenseDate) : new Date());
  ...
  // Check if expense exists on mount
  useEffect(() => {
    if (!expenseToEdit) {
      showAlert('Error', 'We couldn\'t load this expense.', [
        ...
        <Modal transparent visible={isSuccess} animationType="fade">
          <View style={[styles.successOverlay, { backgroundColor: isDark ? 'rgba(15,17,21,0.95)' : 'rgba(255,255,255,0.95)' }]}>
            <Animated.View style={[styles.successCard, successAnimatedStyle, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <Text style={[styles.successText, { color: colors.text }]}>Changes Saved Successfully</Text>
            </Animated.View>
          </View>
        </Modal>
